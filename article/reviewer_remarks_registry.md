...
| --- | --- | --- | --- | --- | --- |
| R1-W1 | Utility of features in GNN backends is unclear; random features appear to replace semantic attributes. | 95% addressed | Added a more cautious interpretation of S2CAG feature modes, explicitly stating that real features can help and that random features are not a replacement for attributes. Added dataset-feature repeated rows for S2CAG from the single-container bundle. | `article/article.tex`: S2CAG dataset-feature stability discussion; S2CAG feature-mode ablation text; `results/icdm-2026-1/measurements/experiment_measurements.json`. | None beyond optional appendix detail. |
| R1-W2 | S2CAG speedup is smaller than contracted workload reduction; overhead source is unclear. | 92% addressed | Added an explicit runtime interpretation: ComNetX overhead is small, while S2CAG is dominated by neural backend cost, feature materialization, sparse tensor construction, and GPU training. The text also explains why A100 80GB GPUs can mask graph-size reduction. | `article/article.tex`: locality/workload reduction discussion around S2CAG overhead; workload profile results in `results/icdm-2026-1/measurements/workload_profiles.json`. | None beyond optional larger-graph follow-up. |
...
| R1-W4 | Limitation to undirected structures and loss of directionality are insufficiently discussed. | 92% addressed | Clarified that main real-data runs explicitly symmetrize graphs for cross-backend comparability, added a Leiden directed-control table for `dyn_pubmed` and `arxivmath`, and narrowed the remaining limitation to direction-preserving objectives and directed baselines beyond Leiden. | `article/article.tex`: experimental setup, directed-control table, limitations; `results/icdm-2026-1/measurements/experiment_measurements.json`. | Remaining limitation is explicit rather than unaddressed: no directed-control rows for every baseline. |
| R1-W5 | Lack of discussion/evaluation of modularity resolution limits under local aggregation. | 90% addressed | Added a targeted Leiden gamma-sensitivity check for `gamma in {0.5, 1, 2}` on `dyn_pubmed` and `arxivmath`. The text states that speedups remain large while high resolution can increase quality sensitivity. | `article/article.tex`: modularity definition, ablation text, limitations; `results/icdm-2026-1/measurements/experiment_measurements.json`. | Optional: add a compact appendix/rebuttal table if page budget permits. Main article currently uses text only. |
...
| R1-Q4 | Why omit local dynamic related work, and how does ComNetX compare? | 86% addressed | Related Work now discusses batch-incremental, local/anchor, overlapping, Delta-Screening, and dynamic Leiden methods. Emphasize solver-agnostic contracted local subproblems; empirical comparison remains limited to reproducibly runnable full-partition baselines. |

### Follow-Up Queue

| Priority | Task | Status |
| --- | --- | --- |
| High | Complete and analyze directed Leiden control on `dyn_pubmed` and `arxivmath`. | 100% addressed |
| High | Replace Table III timing values after the single-container sequential rerun finishes, if variation confirms memory contention. | 100% addressed |
...
- Overall risk: moderate. This review is positive on scope and
  ...
  fourth, and related-work issues with compact text changes; the new
  cut-metric control addresses the structural-quality request, while empirical
  comparison with mismatched local-query baselines remains the main open
  question.
...
| R2-W3 | Modularity dominates the evaluation; structural metrics such as conductance or normalized cut could give a broader view of cluster quality. | 90% addressed | Added a measured structural-quality control, now separated from the directed-control paragraph: mean conductance and normalized cut for full versus Local Leiden on `dyn_pubmed` and `arxivmath` under the same symmetrized `999:10` protocol used by the main real-data comparison. The text states that this is a targeted diagnostic for the main topology backend, not an all-baseline claim. | `article/article.tex`: metrics and structural-quality control paragraph; `results/icdm-2026-1/measurements/cut_metrics.json`. | Optional: extend cut metrics to GNN/native/non-Leiden baselines if page budget and measurement time allow. |
...
| High | Decide whether to compute conductance/normalized-cut. Current manuscript should not mention them unless measured or backed by a clear methodological reason. | 100% addressed |
