...
Уточнение по итогам [аудита кода](code-change-audit-20260911.md): старые
smart-результаты описывают прежнюю процедуру, но counterexample не доказывает,
что каждый запуск имел ошибку. Namespace-дефект возможен и при L=1.
Предварительная фиксация scopes уже была в старом коде. Parent quotients —
выбранная алгоритмическая политика, а не обязательное требование IEEE Access.
План ниже относится к статье о текущей реализации. Старые queue frameworks
и runtime certificates удалены; использовать прямые запускные инструменты.
Все численные выводы и статус рецензий в этом документе относятся к указанному
выше PDF до чистки кода.
...
2. Выполнить fresh corrected ComNetX campaign:
   ...
   - общий full trace переиспользовать между вариантами с одинаковыми
     входами, backend, bootstrap и timing contract; не запускать его заново
     для каждой точки L/r;
