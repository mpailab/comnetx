...
for config in "${CONFIGS[@]}"; do
  ...
  echo "[S2CAG batch and seed sweep] $(date -Is) running $config on CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
  python scripts/launch.py "$config" --paths-config "$PATHS_CONFIG" 2>&1 | tee "$LOG_DIR/gpu2_${name}_${STAMP}.log"
done
