...
log="$LOG_DIR/gpu2_after3_workload_memory_gnn_gpu2_${STAMP}.log"
echo "[workload/memory GNN] $(date -Is) running workload/profile job with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "after3_workload_memory_gnn_gpu2_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[workload/memory GNN] exited with status $status; see $log"
fi
exit "$status"
