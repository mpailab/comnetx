...
log="$LOG_DIR/extra20_after3_dmon_closure_contraction_pubmed_extra20_${STAMP}.log"
echo "[DMoN closure/contraction dyn_pubmed] $(date -Is) running workload/profile job with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "after3_dmon_closure_contraction_pubmed_extra20_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[DMoN closure/contraction dyn_pubmed] exited with status $status; see $log"
fi
exit "$status"
