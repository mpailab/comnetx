...
log="$LOG_DIR/extra17_after3_dmon_feature_modes_pubmed_extra17_${STAMP}.log"
echo "[DMoN feature modes dyn_pubmed] $(date -Is) running workload/profile job with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "after3_dmon_feature_modes_pubmed_extra17_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[DMoN feature modes dyn_pubmed] exited with status $status; see $log"
fi
exit "$status"
