...
log="$LOG_DIR/extra8_after3_dfleiden_closure_contraction_extra8_${STAMP}.log"
echo "[DF-Leiden closure/contraction] $(date -Is) running workload/profile job with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "after3_dfleiden_closure_contraction_extra8_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[DF-Leiden closure/contraction] exited with status $status; see $log"
fi
exit "$status"
