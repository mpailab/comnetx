...
log="$LOG_DIR/retry05_dsbm_hubs_5_batches_mc290_1450_${STAMP}.log"
echo "[retry05_dsbm_hubs_5_batches_mc290_1450] $(date -Is) running focused five-batch DSBM recovery with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/run_dsbm_stress.py \
  ...
  --name "retry05_dsbm_hubs_5_batches_mc290_1450_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[retry05_dsbm_hubs_5_batches_mc290_1450] exited with status $status; see $log"
fi
exit "$status"
