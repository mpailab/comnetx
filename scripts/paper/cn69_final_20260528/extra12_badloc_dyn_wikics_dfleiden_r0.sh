...
log="$LOG_DIR/extra12_final_badloc_dyn_wikics_dfleiden_r0_extra12_${STAMP}.log"
echo "[DF-Leiden r0 full on dyn_wikics] $(date -Is) running optional bad-locality workload profile with timeout=$JOB_TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$JOB_TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "final_badloc_dyn_wikics_dfleiden_r0_extra12_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  mark_manifest_failed "$RESULTS_DIR/manifest_final_badloc_dyn_wikics_dfleiden_r0_extra12_${STAMP}.json" "$status" "DF-Leiden r0 full on dyn_wikics"
  echo "[DF-Leiden r0 full on dyn_wikics] exited with status $status; see $log"
fi
exit "$status"
