...
log="$LOG_DIR/extra22_final_badloc_brain_dfleiden_r2_extra22_${STAMP}.log"
echo "[DF-Leiden r2 full on brain] $(date -Is) running optional bad-locality workload profile with timeout=$JOB_TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$JOB_TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "final_badloc_brain_dfleiden_r2_extra22_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  mark_manifest_failed "$RESULTS_DIR/manifest_final_badloc_brain_dfleiden_r2_extra22_${STAMP}.json" "$status" "DF-Leiden r2 full on brain"
  echo "[DF-Leiden r2 full on brain] exited with status $status; see $log"
fi
exit "$status"
