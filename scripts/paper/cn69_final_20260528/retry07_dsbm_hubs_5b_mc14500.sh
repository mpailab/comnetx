...
for mode in smart naive; do
  run_name="retry07_dsbm_hubs_5_batches_mc14500_${mode}_${STAMP}"
  log="$LOG_DIR/${run_name}.log"
  echo "[retry07_dsbm_hubs_5_batches_mc14500 / $mode] $(date -Is) running focused five-batch DSBM recovery with timeout=$JOB_TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
  ...
  status="${PIPESTATUS[0]}"
  if [[ "$status" -ne 0 ]]; then
    mark_manifest_failed "$RESULTS_DIR/manifest_${run_name}.json" "$status" "retry07_dsbm_hubs_5_batches_mc14500 / $mode"
    echo "[retry07_dsbm_hubs_5_batches_mc14500 / $mode] exited with status $status; see $log"
    FAILED=1
  fi
done
exit "$FAILED"
