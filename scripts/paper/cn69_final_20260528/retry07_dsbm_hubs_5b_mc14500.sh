...
log="$LOG_DIR/retry07_dsbm_hubs_5_batches_mc14500_${STAMP}.log"
echo "[retry07_dsbm_hubs_5_batches_mc14500] $(date -Is) running focused five-batch DSBM recovery with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/run_dsbm_stress.py \
  ...
  --name "retry07_dsbm_hubs_5_batches_mc14500_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[retry07_dsbm_hubs_5_batches_mc14500] exited with status $status; see $log"
fi
exit "$status"
