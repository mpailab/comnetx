...
for mode in smart naive; do
  run_name="retry06_dsbm_hubs_5_batches_mc2900_${mode}_${STAMP}"
  log="$LOG_DIR/${run_name}.log"
  echo "[retry06_dsbm_hubs_5_batches_mc2900 / $mode] $(date -Is) running focused five-batch DSBM recovery with timeout=$JOB_TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
  ...
  status="${PIPESTATUS[0]}"
  if [[ "$status" -ne 0 ]]; then
    mark_manifest_failed "$RESULTS_DIR/manifest_${run_name}.json" "$status" "retry06_dsbm_hubs_5_batches_mc2900 / $mode"
    echo "[retry06_dsbm_hubs_5_batches_mc2900 / $mode] exited with status $status; see $log"
    FAILED=1
  fi
done
exit "$FAILED"
