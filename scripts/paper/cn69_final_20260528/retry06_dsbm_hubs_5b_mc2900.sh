...
log="$LOG_DIR/retry06_dsbm_hubs_5_batches_mc2900_${STAMP}.log"
echo "[retry06_dsbm_hubs_5_batches_mc2900] $(date -Is) running focused five-batch DSBM recovery with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/run_dsbm_stress.py \
  ...
  --name "retry06_dsbm_hubs_5_batches_mc2900_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[retry06_dsbm_hubs_5_batches_mc2900] exited with status $status; see $log"
fi
exit "$status"
