...
log="$LOG_DIR/extra17_final_badloc_brain_leiden_r1_extra17_${STAMP}.log"
echo "[Leiden r1 full on brain] $(date -Is) running optional bad-locality workload profile with timeout=$JOB_TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$JOB_TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "final_badloc_brain_leiden_r1_extra17_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  mark_manifest_failed "$RESULTS_DIR/manifest_final_badloc_brain_leiden_r1_extra17_${STAMP}.json" "$status" "Leiden r1 full on brain"
  echo "[Leiden r1 full on brain] exited with status $status; see $log"
fi
exit "$status"
