...
log="$LOG_DIR/gpu2_pilot_dsbm_update_size_gpu2_${STAMP}.log"
echo "[DSBM update-size pilot] $(date -Is) running DSBM job with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/run_dsbm_stress.py \
  ...
  --name "pilot_dsbm_update_size_gpu2_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[DSBM update-size pilot] exited with status $status; checkpointed files, if any, are in results/paper_icdm"
fi
exit "$status"
