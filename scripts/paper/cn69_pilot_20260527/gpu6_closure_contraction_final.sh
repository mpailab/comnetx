...
log="$LOG_DIR/gpu6_final_closure_contraction_gpu6_${STAMP}.log"
echo "[closure/contraction final] $(date -Is) running workload/profile job with timeout=$TIMEOUT on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-container-bound}"
timeout --kill-after=2m "$TIMEOUT" python scripts/paper/profile_smart_workload.py \
  ...
  --name "final_closure_contraction_gpu6_${STAMP}" \
  2>&1 | tee "$log"
status="${PIPESTATUS[0]}"
if [[ "$status" -ne 0 ]]; then
  echo "[closure/contraction final] exited with status $status; see $log"
fi
exit "$status"
