...
echo "[INFO] Installing system packages (Python 3.10, pip, build deps, graph-tool deps)..."
$DOCKER exec "$NAME" bash -lc "
    ...
    add-apt-repository -y ppa:graph-tool/graph-tool &&
    apt-get update &&
    apt-get install -y python3-graph-tool
"

echo "[INFO] Installing Python packages for PRGPT..."
$DOCKER exec "$NAME" bash -lc "
    set -e
...
echo "[INFO] All Python packages for PRGPT installed."
