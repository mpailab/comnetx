...
def compute_neighborhood_sizes(dataset_name, batches_strategy, max_step=5, skip_first_batch=True):
    ...
    Args:
        ...
        batches_strategy: стратегия батчинга в формате "p:n" или "n"
...
def main(max_nodes=None, min_nodes=None, max_edges=None, min_edges=None, 
    ...
    for dataset_name in datasets_by_nodes:
        ...
        for p in p_values:
            for n in n_values:
                strategy = f"{p}:{n}" if p != 0 else f"{n}"
...
if __name__ == "__main__":
    ...
    Примеры стратегий:
      ...
      "0:10" - равномерное разбиение на 10 батчей
    """
    ...
    3. Использовать только определенные стратегии, включая p=0:
       python compute_neighborhood_sizes.py --p-values 0 9 99 --n-values 10 100
    ...
    parser.add_argument('--p-values', type=int, nargs='+', default=[9, 99],
                       help='Значения p для стратегий p:n (можно указывать 0 для равномерного разбиения)')
    ...
    # Проверяем корректность значений p
    if not all(p in {0, 9, 99, 999} for p in args.p_values):
        print("Ошибка: возможные значения для p - 0, 9, 99, 999")
        sys.exit(1)
